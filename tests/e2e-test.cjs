...
(async () => {
  ...
  elements.forEach((e) =>
    console.log(
      `  ${e.anim} (${e.trigger}): opacity=${e.opacity}, animation=${e.animation}, init=${e.hasInit}`,
    ),
  );
  ...
  loadEls.forEach((e) =>
    console.log(`  ${e.anim}: style.animation="${e.style}", computed="${e.computedAnim}"`),
  );
  ...
  console.log(
    '  Names:',
    keyframesExist.slice(0, 10).join(', '),
    keyframesExist.length > 10 ? '...' : '',
  );
