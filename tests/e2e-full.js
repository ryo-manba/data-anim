...
(async () => {
  ...
  const keyframeCount = await page.evaluate(() => {
    ...
    for (const sheet of document.styleSheets) {
      try { for (const rule of sheet.cssRules) { if (rule instanceof CSSKeyframesRule) count++; } } catch(e) {}
    }
    return count;
  });
    ...
    for (const sheet of document.styleSheets) {
      try {
        for (const rule of sheet.cssRules) {
          if (rule.selectorText && rule.selectorText.includes('data-anim') && rule.style.opacity === '0') return true;
        }
      } catch(e) {}
    }
    return false;
  ...
  const hasReducedMotion = await page.evaluate(() => {
    for (const sheet of document.styleSheets) {
      try {
        for (const rule of sheet.cssRules) {
          if (rule instanceof CSSMediaRule && rule.conditionText && rule.conditionText.includes('prefers-reduced-motion')) return true;
        }
      } catch(e) {}
    }
    return false;
  ...
  assert('Hover trigger: animation resets on mouseleave', shakeAfterLeave === 'none', `got "${shakeAfterLeave}"`);
  ...
  assert('Load trigger: fadeInUp plays', loadAnims[0]?.animName === 'da-fadeInUp', `got ${loadAnims[0]?.animName}`);
  ...
  const loadDelays = await page.evaluate(() => {
    return Array.from(document.querySelectorAll('[data-anim-trigger="load"]')).map(el => el.style.animation);
  });
  assert('Load trigger: delay=200ms on zoomIn', loadDelays[1]?.includes('200ms') || loadDelays[1]?.includes('0.2s'), `got "${loadDelays[1]}"`);
  ...
  assert('Load trigger: duration is reasonable', loadDuration === '1.6s' || loadDuration === '1600ms' || loadDuration === '0.8s' || loadDuration === '800ms', `got ${loadDuration}`);
  ...
  const scrollAnims = await page.evaluate(() => {
    return Array.from(document.querySelectorAll('[data-anim-trigger="scroll"], [data-anim]:not([data-anim-trigger])')).filter(el => !el.getAttribute('data-anim-trigger')).map(el => ({
      anim: el.getAttribute('data-anim'),
      animName: getComputedStyle(el).animationName,
  ...
  scrollAnims.forEach(e => {
    assert(`Scroll trigger: ${e.anim} animates after scroll`, e.animName.includes('da-') || e.opacity === '1', `animName=${e.animName}, opacity=${e.opacity}`);
  });
  ...
  const sections = await page.evaluate(() => {
    return ['hero', 'what', 'features', 'how', 'gallery', 'comparison', 'quickstart', 'footer'].map(id => !!document.getElementById(id));
  });
  assert('LP: All 8 sections exist', sections.every(Boolean), `missing: ${sections.map((v,i) => v ? null : ['hero','what','features','how','gallery','comparison','quickstart','footer'][i]).filter(Boolean).join(', ')}`);
  ...
  if (galleryCardEl) {
    ...
    assert('LP: Gallery hover animation plays', galleryHoverAnim.includes('da-'), `got ${galleryHoverAnim}`);
  } else {
  ...
  assert('LP: No horizontal overflow at 375px', mobileOk, `scrollWidth=${await page.evaluate(() => document.body.scrollWidth)}`);
  ...
  const hasCopyBtn = await page.evaluate(() => {
    return Array.from(document.querySelectorAll('[onclick]')).some(el => el.getAttribute('onclick')?.includes('clipboard'));
  });
  ...
  const hasSkipLink = await page.evaluate(() => !!document.querySelector('a[href="#main"], .skip-link'));
  ...
  const scrolledAnims = await page.evaluate(() => {
    return Array.from(document.querySelectorAll('[data-anim]:not([data-anim-trigger="load"]):not([data-anim-trigger="hover"]):not([data-anim-trigger="click"])')).filter(el => {
  ...
  scrolledAnims.forEach(e => {
    assert(`LP scroll: ${e.anim} visible after scroll`, e.opacity === '1' || parseFloat(e.opacity) > 0.5);
  });
  ...
  for (const path of docsPages) {
    const resp = await page.goto(`http://localhost:3333/docs/dist${path}`, { waitUntil: 'domcontentloaded' }).catch(() => null);
    const status = resp?.status() || 0;
    assert(`Docs page loads: ${path}`, status === 200, `status=${status}`);
  }

  // Docs index page
  const docsIndex = await page.goto('http://localhost:3333/docs/dist/', { waitUntil: 'domcontentloaded' }).catch(() => null);
