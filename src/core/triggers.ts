...
export function setupTrigger(el: DataAnimElement): void {
  ...
  if (t === 'load') {
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => playAnimation(el), { once: true });
    else playAnimation(el);
  } else if (t === 'click') {
    el.addEventListener('click', () => { el.style.animation = 'none'; void el.offsetWidth; playAnimation(el); });
  } else if (t === 'hover') {
    let animating = false;
    el.addEventListener('mouseenter', () => {
      if (animating) return;
      animating = true;
      el.style.animation = 'none'; void el.offsetWidth; playAnimation(el, true);
    ...
    el.addEventListener('blur', () => { if (!animating) resetAnimation(el, true); });
  } else {
    createObserver(el);
  }
}
