...
export function playAnimation(el: DataAnimElement, isHover = false): void {
  ...
  const c = getConfig(), dist = el.getAttribute('data-anim-distance');
  ...
  debugLog('play:' + n, el); debugOutline(el);
}
...
export function resetAnimation(el: DataAnimElement, forHover = false): void {
  ...
  s?.initialStyle?.split(';').forEach(r => { const [p, v] = r.split(':'); if (p && v) el.style.setProperty(p.trim(), v.trim()); });
}

export function createObserver(el: DataAnimElement): void {
  const c = getConfig(), th = parseFloat(el.getAttribute('data-anim-offset') || '') || c.offset;
  const once = el.hasAttribute('data-anim-once') || c.once, mirror = el.hasAttribute('data-anim-mirror');
  const o = new IntersectionObserver(es => { es.forEach(e => {
    if (e.isIntersecting) { playAnimation(el); if (once) o.unobserve(el); }
    else if (mirror && el._daInit) resetAnimation(el);
  }); el._daInit = true; }, { threshold: th });
  el._daObs = o; o.observe(el);
}
